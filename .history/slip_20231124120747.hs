...
check env (Labs x e) (Tabs t1 t2) =
    check ((x, t1) : env) e t2

check env (Lid x) t = case mlookup env x of
                        Tunknown -> ["Variable inconnue: " ++ show x]
                        t' -> (["Variable \"" ++ show x
                                       ++ "\" a type \"" ++ show t'
                                       ++ "\" au lieu de: " ++ show t | t /= t'])



check env (Ldec x e1 e2) t =
    case synth env e1 of
      (t1, errors) -> errors ++ check ((x, t1) : env) e2 t
check env (Lfuncall e es) _ =
    case synth env e of
      (Tabs t1 t2, errors) ->
          errors ++ check env e (Tabs t1 t2)
          ++ concatMap (\x_ -> check env x_ t1) es
      (Tunknown, errors) -> errors
      (t', errors) -> ("Pas une fonction: " ++ show t') : errors
...
synth env (Labs x e) =
    case synth ((x, Tint) : env) e of
      (t1, errors) -> (Tabs Tint t1, errors)

synth env (Ldec x e1 e2) =
    case synth env e1 of
      (t1, errors) -> (t1, errors ++ check ((x, t1) : env) e2 t1)

synth env (Lfuncall e es) =
    case synth env e of
      (Tbool , errors) ->
          (Tunknown, errors ++ concatMap (\x_ -> check env x_ Tbool) es)
      (Tabs t1 t2, errors) ->
          (t2, errors ++ concatMap (\x_ -> check env x_ t1) es)
      (Tunknown, errors) -> (Tunknown, errors)
      (t', errors) -> (Tunknown, ("Pas une fonction: " ++ show t') : errors)
