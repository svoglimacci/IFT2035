...
synth env (Lfuncall e1 e2) =
  let (t, errors) = synth env e1
  in case t of
    Tabs t1 t2 ->
      let te = concatMap (\e -> check env e t1) e2
      in if not (null te)
         then error (head te)
         else if t2 == Tunknown
              then (t2, errors ++ te)
              else (t2, errors)
    _ -> (Tunknown, errors ++ ["Function type expected"])

synth _ (Labs _ _) = error "Impossible de synthétiser le type d'une fonction."
