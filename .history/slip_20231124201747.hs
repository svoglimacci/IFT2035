...
synth env (Lfuncall (Labs x e) []) =
  let (t, errors) = synth ((x, Tunknown) : env) e
  in case t of
    Tunknown -> (Tunknown, errors)
    _ -> (t, errors)

synth env (Lfuncall (Labs x e) (arg:[])) =
  let (t, errors) = synth ((x, Tunknown) : env) e
  in case t of
    Tunknown -> (Tunknown, errors)
    _ -> synth env arg

synth env (Lfuncall (Labs x e) (arg1 : args)) =
  let (t, errors) = synth ((x, Tunknown) : env) e
  in case t of
    Tunknown -> (Tunknown, errors)
    _ -> synth env (Lfuncall (Labs x e) args)
