...
synth env (Lite e1 e2 e3) =
  let (t1, errors1) = synth env e1
      ...
      (t3, errors3) = synth env e3
  in if t1 == Tbool
     then if t2 == t3
          then (t2, errors1 ++ errors2 ++ errors3)
          else (Tunknown, errors1 ++ errors2 ++ errors3 ++ ["Branches de type différent"])
     else (Tunknown, errors1 ++ errors2 ++ errors3 ++ ["Condition non booléenne"])
