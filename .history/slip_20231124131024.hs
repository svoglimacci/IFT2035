...
synth env (Lfuncall e es) = case synth env e of
    (Tabs t1 t2, errors) -> (t2, errors ++ concatMap (\e1 -> check env e1 t1) es)
    (Tunknown, errors) -> (Tunknown, errors ++ concatMap (\e1 -> check env e1 Tunknown) es)
    (t, errors) -> (Tunknown, errors ++ concatMap (\e1 -> check env e1 Tunknown) es)
