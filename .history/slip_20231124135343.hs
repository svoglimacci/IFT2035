...
check env (Lfuncall e es) t = check env e (Tabs t Tunknown) ++ concatMap (\e1 -> check env e1 Tunknown) es
...
-- (+ 2 4) : Tint
-- if  Tabs Tint then Tint
synth env (Lfuncall e es) =
    let (t, errors) = synth env e
    in case t of
      Tint -> (Tint, errors ++ concatMap (\e1 -> check env e1 Tint) es)
      Tbool -> (Tbool, errors ++ concatMap (\e1 -> check env e1 Tbool) es)
      Tabs t1 t2 -> (t2, errors ++ concatMap (\e1 -> check env e1 t1) es)
      _ -> (Tunknown, errors ++ concatMap (\e1 -> check env e1 Tunknown) es)
